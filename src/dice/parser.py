...
from dice.ast_nodes import Add, BinaryNode, Call, Compare, Const, Die, Expr, IfElse, Mul, Or, And, Sub, Var, VALID_OPS
...
class _Parser:
    ...
    def _consume(self, kind: TokenKind | None = None, value: str | None = None) -> Token:
        ...
        if kind is not None and token.kind != kind:
            raise SyntaxError(f'Expected token kind {kind.name!r} but got {token.kind.name!r} ({token.value!r})')
...
def _parse_function_def(stmt_tokens: list[Token]) -> tuple[str, list[str], Expr]:
    ...
    if not parser._at_end():
        ...
        raise SyntaxError(f'Unexpected trailing token in function body: {trailing.kind.name!r} ({trailing.value!r})')
    ...
    match expr:
        case Call(name=name, args=args):
            ...
            if len(args) != len(param_names):
                raise ValueError(f'Function {name!r} takes {len(param_names)} argument(s), got {len(args)}')
...
def parse(text: str) -> tuple[Expr, dict[str, Domain]]:
    ...
    for stmt_tokens in statement_token_lists[:-1]:
        if not _is_function_definition(stmt_tokens):
            raise SyntaxError('Non-final statements must be function definitions of the form name(params): expr')
